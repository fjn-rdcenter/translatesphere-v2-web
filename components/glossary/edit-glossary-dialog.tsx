...
export function EditGlossaryDialog({
    ...
    reader.onload = (e) => {
      ...
      if (newTerms.length > 0) {
        ...
        const seenSources = new Set(
            terms.filter(t => !t.isDeleted).map(t => t.source)
        );

        newTerms.forEach(term => {
            const normalizedSource = term.source;
        ...
        if (addedCount > 0) {
            ...
            let message = `${addedCount} terms imported successfully.`;
            if (skippedCount > 0) {
            ...
            toast({
             ...
             toast({
              variant: "destructive",
              title: "Import Failed",
              description: `All ${newTerms.length} terms were duplicates and have been skipped.`,
            });
            event.target.value = "";
        }
      } else {
  ...
  const undoDelete = (termId: string) => {
      setTerms(prev => prev.map(t => t.id === termId ? { ...t, isDeleted: false } : t));
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const edited = terms.filter(t => t.isEdited && !t.isNew && !t.isDeleted);
      if (edited.length > 0) {
           await Promise.all(
              edited.map(t => 
  ...
  const hasChanges =
    (glossary && (name !== glossary.name || description !== (glossary.description || ""))) ||
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-1">
                <DialogTitle className="text-xl font-semibold tracking-tight flex items-center gap-2">
                    Edit Glossary
                    {hasChanges && <Badge variant="secondary" className="text-[10px] h-5 px-1.5">Unsaved Changes</Badge>}
                </DialogTitle>
                <DialogDescription>
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={cn(
                                                ...
                                                isDeleted ? "opacity-60 bg-red-50/50 border-red-200" : "hover:border-primary/30",
                                                isEdited && !isDeleted ? "border-amber-400/50 bg-amber-50/20" : ""
                                            )}
                                        >
                                            ...
                                            <MoveRight className={cn("w-3.5 h-3.5 text-muted-foreground/30 mx-auto", isDeleted && "opacity-50")} />
                                            ...
                                            {/* Status Indicator */}
                                            {isEdited && !isDeleted && (
                                                <div className="absolute right-12 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-amber-400" title="Modified" />
                                            )}
                                            {term.isNew && !isDeleted && (
                                                 <div className="absolute right-12 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-green-500" title="New" />
                                            )}
                                        </div>
                                    );
                                })
        ...
        <DialogFooter className="p-4 bg-white border-t shrink-0 z-20">
             <div className="flex items-center justify-between w-full">
                ...
                <div className="flex gap-2">
                    ...
                    <Button onClick={handleSave} disabled={!hasChanges || isSaving || loading} className="px-8 min-w-[140px]">
                        {isSaving ? <Loader2 className="mr-2 w-4 h-4 animate-spin" /> : <Save className="w-4 h-4 mr-2" />}
                        Save Changes
                    </Button>
                </div>
             </div>
